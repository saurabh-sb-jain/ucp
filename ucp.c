...
int main(int argc, char **argv)
{
    ...
    while ((ch = getopt(argc, argv, "dn:s:")) != -1)
    {
        ...
        {
        case 'n':
            oper.num_bufs = atoi(optarg);
            break;
        case 's':
            oper.bufsize = atol(optarg);
            break;
        case 'd':
            debug = true;
            break;
    ...
    if (oper.num_bufs > DFLT_NUM_BUFS)
    {
        oper.queue_sz = oper.num_bufs;
    }
